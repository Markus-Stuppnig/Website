  ...
  return (
    <section id="about" className="flex justify-center w-full">
		<div className="flex flex-col justify-center items-center w-full text-center">
			<SectionHeading additional="mb-[2rem] font-semibold" textColor="text-secondary" size="text-[2.2rem]">Recommendations</SectionHeading>
			...
			{recommendationsData.map((recommendation, index) => (
				<div key={`recommendation${index}`} className="flex flex-col w-full justify-center items-center bg-secondary shadow shadow-3xl py-[6rem] text-white">
					<div className="flex flex-col items-center justify-center w-11/12 md:w-[45rem] lg:w-[60rem] bg-white rounded-3xl shadow-xl backdrop-filter backdrop-blur-lg bg-opacity-20">
						<div className="flex flex-col ml-[1rem] md:ml-[3rem] text-start">
							<div className="flex flex-row mt-[1rem] md:mt-10 justify-between">
								<Link href={recommendation.person.link} target="_blank" className="flex flex-row">
									...
									<div className="flex flex-col text-start ml-5">
										<h2 className="text-md font-medium text-lg md:text-2xl">{recommendation.person.name}</h2>
										<h2 className="text-md font-medium text-md md:text-xl">{recommendation.person.position}</h2>
										<h2 className="text-sm font-medium text-md md:text-xl sm:font-normal 2xl:font-medium">{recommendation.person.additionalInformation}</h2>
									</div>
								</Link>
								<Link href={recommendation.person.link} target="_blank" className="mr-10 hidden md:block">
